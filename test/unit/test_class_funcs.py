...
class TKinterTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ex_phrase_concise = 'I will be concise. I want to be concise.' \
                                 ' I hope to one day be concise.'
        self.ex_phrase_terse = 'I will be terse. I want to be terse. ' \
                               'I hope to one day be terse.'
...
class TestClassFuncs(TKinterTestCase):
    ...
    def test_get_child_rep_tag(self):
        ...
        expected = self.ex.repFlag + str(idx) + self.ex.repIdFlag + str(sync) \
            + self.ex.syncFlag + pattern
        self.assertEqual(expected, self.ex.get_child_rep_tag(idx,
                                                             sync,
                                                             pattern))
        self.assertEqual(expected, self.ex.get_child_rep_tag(str(idx),
                                                             str(sync),
                                                             pattern)
                         )
    ...
    def test_get_synced_tags(self):
        ...
        self.assertListEqual(self.ex.get_synced_tags([self.sample_rep_s1,
                                                      self.sample_rep_s1]),
                             [self.sample_rep_s1, self.sample_rep_s1])
        self.assertListEqual(self.ex.get_synced_tags([self.sample_rep_u1,
                                                      self.sample_rep_s1]),
                             [self.sample_rep_s1])
        self.assertListEqual(self.ex.get_synced_tags([self.sample_rep_s1,
                                                      self.sample_rep_u1]),
    ...
    def test_setup_rep_bind_tag_attacker(self):
        ...
        for t in attacker_tags:
            ...
            event_gen.click_char(self.ex,
                                 char_idx,
                                 event_gen.right_click_coord)
    ...
    def test_gen_options(self):
        ...
        self.assertEqual(option_list_typebox.cget('text'),
                         self.ex.get_replace_type(attacker_tag)[0])
    ...
    def test_new_option(self):
        ...
        self.ex.add_tag_rep(self.synonyms[0],
                            self.synonyms,
                            sync=self.ex.syncTrue)
        self.assertEqual(self.ex.get('1.0', 'end').splitlines()[0],
                         self.ex_phrase_concise)
        ...
        self.ex.update_idletasks()
        event_gen.left_click_coord(ex_opt,
                                   [ex_opt.winfo_x(), ex_opt.winfo_y()])
        self.assertEqual(self.ex.get('1.0', 'end').splitlines()[0],
                         self.ex_phrase_terse)
    ...
    def test_gen_changing_typebox_get_to_fro(self):
        ex_iso = self.sample_iso
        ex_rep_s = self.sample_rep_s1
        ex_rep_u = self.sample_rep_u1
        ex_iso_ret = self.ex.gen_changing_typebox_get_to_fro(ex_iso)
        ex_rep_s_ret = self.ex.gen_changing_typebox_get_to_fro(ex_rep_s)
        ex_rep_u_ret = self.ex.gen_changing_typebox_get_to_fro(ex_rep_u)
        self.assertEqual(ex_iso_ret[0], self.ex.replace_types[0])
        self.assertEqual(ex_iso_ret[1], self.ex.replace_types[0])
        self.assertEqual(ex_rep_s_ret[0], self.ex.replace_types[1])
        self.assertEqual(ex_rep_s_ret[1], self.ex.replace_types[2])
        self.assertEqual(ex_rep_u_ret[0], self.ex.replace_types[2])
        self.assertEqual(ex_rep_u_ret[1], self.ex.replace_types[1])
    ...
    def test_replace_text(self):
        ...
        self.ex.replace_text("foo", self.sample_iso)
        self.assertEqual(self.ex.get('2.0', 'end-1c'),
                         'foo' + self.sample_line[10:])
        self.ex.replace_text("foo", self.sample_iso)
        self.assertEqual(self.ex.get('2.0', 'end-1c'),
                         'foo' + self.sample_line[10:])
        self.ex.replace_text("bar", self.sample_iso)
        self.assertEqual(self.ex.get('2.0', 'end-1c'),
                         'bar' + self.sample_line[10:])
    ...
    def test_replace_texts(self):
        ...
        self.assertEqual(self.ex.get('2.0', 'end-1c'),
                         'bar' + 'bar' + self.sample_line[20:])
        self.ex.replace_texts('foo',
                              [self.sample_iso,
                               self.sample_rep_s1,
                               self.sample_rep_u1])
        self.assertEqual(self.ex.get('2.0', 'end-1c'),
    ...
    def test_add_tag_rep(self):
        # This test only checks that all of the expected sequences are binded
        # to each tagname, as how tkinter remembers what functions are binded
        # to each sequence is very hard to interpret
        ex_parent = self.ex.get_parent_rep_tag(self.synonyms[0])
        self.ex.add_tag_rep(self.synonyms[0],
                            self.synonyms,
                            sync=self.ex.syncTrue)
    ...
    def test_setup_rep_bind_attacker(self):
        ...
        self.ex.setup_rep_bind_tag_attacker(self.sample_rep_u1,
                                            [self.sample_rep_u1],
                                            self.sample_parent)
        expected_seqs = ['<Button-2>', '<Shift-Button-2>', '<Shift-Button-1>']
